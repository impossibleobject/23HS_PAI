...
class Model(object):
    ...
    def __init__(self, kernel=None):
        ...
        self.regressor = GaussianProcessRegressor(kernel=kernel, n_restarts_optimizer=5, normalize_y=True)
    ...
    def make_predictions(self, test_x_2D: np.ndarray, test_x_AREA: np.ndarray) -> typing.Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        print(f"predictions: {gp_mean}")
        
        predictions = np.maximum(0, gp_mean)
        
        #map_city_area = lambda t : t[1]+4 if test_x_AREA[t[0]] else t[1]
        #predictions = np.array([map_city_area(t) for t in enumerate(predictions)])
        print(predictions)
    ...
    def fitting_model(self, train_y: np.ndarray,train_x_2D: np.ndarray):
        ...
        #sample_idxs = rng.choice(train_y.shape[0], size=train_y.shape[0]//10, replace=False)
        sorted_idxs = np.argsort(train_y)[-train_y.shape[0]//3:]
        print(train_y[sorted_idxs])


        self.regressor = self.regressor.fit(train_x_2D[sorted_idxs], train_y[sorted_idxs])
